...
console.log(accounts[1]);
        await makeTransaction(referral, signAddress,  crowdsale.address, accounts[1], new BigNumber('1000').mul(precision).valueOf())
            .then(Utils.receiptShouldSucceed);
    ...
    it("only  crowdsale signers  can run it", async function () {
        await makeTransaction(referral, signAddress,  crowdsale.address, accounts[1], new BigNumber('1000').mul(precision).valueOf())
            .then(Utils.receiptShouldSucceed);
        await makeTransaction(referral, accounts[4],  crowdsale.address, accounts[1], new BigNumber('1000').mul(precision).valueOf())
            .then(Utils.receiptShouldFailed)
            .catch(Utils.catchReceiptShouldFailed)
    })
    ...
    it("if  sentOnce is  false referral can claim tokens many times", async function () {
        await makeTransaction(referral, signAddress,  crowdsale.address, accounts[1], new BigNumber('100').mul(precision).valueOf())
            .then(Utils.receiptShouldSucceed);
        await makeTransaction(referral, signAddress,  crowdsale.address, accounts[1], new BigNumber('100').mul(precision).valueOf())
            .then(Utils.receiptShouldSucceed);
    })
